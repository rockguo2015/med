...
public class PMWorkItem4TeamListGrid extends GXTGridComponent<UIPMWorkItem> implements IInitializerSupport {
  @Inject
  ...
  protected PMWorkItem4TeamListGrid doSetValue(final UIPMWorkItem value) {
    this.value = value;
  ...
  public void initializeComponent() {
    ...
    final Procedure3<Context,UIPMWorkItemStatue,SafeHtmlBuilder> _function = new Procedure3<Context,UIPMWorkItemStatue,SafeHtmlBuilder>() {
        public void apply(final Context context, final UIPMWorkItemStatue value, final SafeHtmlBuilder sb) {
          final String stateStr = value.toString();
          boolean _or = false;
          boolean _or_1 = false;
          boolean _equals = Objects.equal(value, UIPMWorkItemStatue.teamAssigned);
          if (_equals) {
            _or_1 = true;
          } else {
            boolean _equals_1 = Objects.equal(value, UIPMWorkItemStatue.partialExecuting);
            _or_1 = (_equals || _equals_1);
          }
          if (_or_1) {
            _or = true;
          } else {
            boolean _equals_2 = Objects.equal(value, UIPMWorkItemStatue.ready4TeamFinishReport);
            _or = (_or_1 || _equals_2);
          }
          if (_or) {
            String _plus = ("<B style=\'color:red;\'>" + stateStr);
