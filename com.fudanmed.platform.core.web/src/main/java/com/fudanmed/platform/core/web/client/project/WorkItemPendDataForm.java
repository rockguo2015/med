...
public class WorkItemPendDataForm extends GXTFormComponent<WorkItemPendData> implements IInitializerSupport {
  @Inject
  ...
  public void initialize(final IPostInitializeAction handler) {
    java.util.Collection<com.uniquesoft.gwt.client.common.async.IInitializerSupport> _initializerSupports = com.google.common.collect.Lists.newArrayList();finishDate = dateValueProviderFactory.createAsDateItem("finishDate","挂起日期");
    widgetsRegistory.put("finishDate",finishDate);
    finishTime = dateValueProviderFactory.createAsTimeItem("finishTime","挂起时间");
  ...
  public WorkItemPendData getValueAsNew() {
    WorkItemPendData value = new WorkItemPendData();
  ...
  public WorkItemPendData doGetValue() {
  ...
  public WorkItemPendData getValue(final WorkItemPendData copyValue) {
  ...
  protected WorkItemPendDataForm doSetValue(final WorkItemPendData value) {
    this.value = value;
  ...
  public Widget asWidget() {
    final Function1<Void,VerticalLayoutContainer> _function = new Function1<Void,VerticalLayoutContainer>() {
        public VerticalLayoutContainer apply(final Void it) {
          VerticalLayoutContainer _VLayout = WorkItemPendDataForm.this.widgets.VLayout();
          final Procedure1<VerticalLayoutContainer> _function = new Procedure1<VerticalLayoutContainer>() {
              public void apply(final VerticalLayoutContainer it) {
                FieldLabel _asWidget = WorkItemPendDataForm.this.finishDate.asWidget();
                int _minus = (-1);
                VerticalLayoutData _VLayoutData = WorkItemPendDataForm.this.widgets.VLayoutData(1, _minus);
                it.add(_asWidget, _VLayoutData);
                FieldLabel _asWidget_1 = WorkItemPendDataForm.this.finishTime.asWidget();
                int _minus_1 = (-1);
                VerticalLayoutData _VLayoutData_1 = WorkItemPendDataForm.this.widgets.VLayoutData(1, _minus_1);
                it.add(_asWidget_1, _VLayoutData_1);
                FieldLabel _asWidget_2 = WorkItemPendDataForm.this.comment.asWidget();
                VerticalLayoutData _VLayoutData_2 = WorkItemPendDataForm.this.widgets.VLayoutData(366, 80);
