...
public class GroupTaskMisDispatchReportDataForm extends GXTFormComponent<GroupTaskMisDispatchReportData> implements IInitializerSupport {
  @Inject
  ...
  public void initialize(final IPostInitializeAction handler) {
    java.util.Collection<com.uniquesoft.gwt.client.common.async.IInitializerSupport> _initializerSupports = com.google.common.collect.Lists.newArrayList();finishDate = dateValueProviderFactory.createAsDateItem("finishDate","串单上报日期");
    widgetsRegistory.put("finishDate",finishDate);
    finishTime = dateValueProviderFactory.createAsTimeItem("finishTime","串单上报时间");
  ...
  public GroupTaskMisDispatchReportData getValueAsNew() {
    GroupTaskMisDispatchReportData value = new GroupTaskMisDispatchReportData();
  ...
  public GroupTaskMisDispatchReportData doGetValue() {
  ...
  public GroupTaskMisDispatchReportData getValue(final GroupTaskMisDispatchReportData copyValue) {
  ...
  protected GroupTaskMisDispatchReportDataForm doSetValue(final GroupTaskMisDispatchReportData value) {
    this.value = value;
  ...
  public Widget asWidget() {
    final Function1<Void,VerticalLayoutContainer> _function = new Function1<Void,VerticalLayoutContainer>() {
        public VerticalLayoutContainer apply(final Void it) {
          VerticalLayoutContainer _VLayout = GroupTaskMisDispatchReportDataForm.this.widgets.VLayout();
          final Procedure1<VerticalLayoutContainer> _function = new Procedure1<VerticalLayoutContainer>() {
              public void apply(final VerticalLayoutContainer it) {
                FieldLabel _asWidget = GroupTaskMisDispatchReportDataForm.this.finishDate.asWidget();
                int _minus = (-1);
                VerticalLayoutData _VLayoutData = GroupTaskMisDispatchReportDataForm.this.widgets.VLayoutData(1, _minus);
                it.add(_asWidget, _VLayoutData);
                FieldLabel _asWidget_1 = GroupTaskMisDispatchReportDataForm.this.finishTime.asWidget();
                int _minus_1 = (-1);
                VerticalLayoutData _VLayoutData_1 = GroupTaskMisDispatchReportDataForm.this.widgets.VLayoutData(1, _minus_1);
                it.add(_asWidget_1, _VLayoutData_1);
                FieldLabel _asWidget_2 = GroupTaskMisDispatchReportDataForm.this.comment.asWidget();
                VerticalLayoutData _VLayoutData_2 = GroupTaskMisDispatchReportDataForm.this.widgets.VLayoutData(366, 80);
